...
class Base
{
    ...
    public static function randomElements($array = array('a', 'b', 'c'), $count = 1, $allowDuplicates = false)
    {
        ...
        if (!$allowDuplicates AND $numKeys < $count) {
    ...
    {
        if (!$array || ($array instanceof \Traversable AND !count($array))) {
            return null;
        }
    ...
    public function optional($weight = 0.5, $default = null)
    {
        ...
        // TODO: remove in v2
        if ($weight > 0 AND $weight < 1 AND mt_rand() / mt_getrandmax() <= $weight) {
            return $this->generator;
        }

        // new system with percentage
        if (is_int($weight) AND mt_rand(1, 100) <= $weight) {
            return $this->generator;
        }
