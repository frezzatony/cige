...
class PHPMailer
{
    ...
    protected function edebug($str)
    {
        ...
        if (is_callable($this->Debugoutput) AND !in_array($this->Debugoutput, ['error_log', 'html', 'echo'])) {
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (static::idnSupported() AND $this->has8bitChars(substr($address, ++$pos))) {
            if ('Reply-To' !== $kind) {
    ...
    public static function parseAddresses($addrstr, $useimap = true)
    {
        ...
        if ($useimap AND function_exists('imap_rfc822_parse_adrlist')) {
            ...
            foreach ($list as $address) {
                if (('.SYNTAX-ERROR.' !== $address->host) AND static::validateAddress(
    ...
    public function preSend()
    {
        ...
        if ('mail' === $this->Mailer
            AND ((PHP_VERSION_ID >= 70000 AND PHP_VERSION_ID < 70017)
                || (PHP_VERSION_ID >= 70100 AND PHP_VERSION_ID < 70103))
            AND ini_get('mail.add_x_header') === '1'
    ...
    protected function mailSend($header, $body)
    {
        ...
        // CVE-2016-10033, CVE-2016-10045: Don't pass -f if characters will be escaped.
        if (!empty($this->Sender) AND static::validateAddress($this->Sender) AND self::isShellSafe($this->Sender)) {
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('ssl' === $hostinfo[2] || ('' === $hostinfo[2] AND static::ENCRYPTION_SMTPS === $this->SMTPSecure)) {
                $prefix = 'ssl://';
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    if ($this->SMTPAuth AND !$this->smtp->authenticate(
                        $this->Username,
                        $this->Password,
                        $this->AuthType,
                        $this->oauth
    ...
    {
        if ((null !== $this->smtp) AND $this->smtp->connected()) {
            $this->smtp->quit();
            $this->smtp->close();
        }
    }
        ...
        foreach ($lines as $line) {
            ...
            foreach ($words as $word) {
                if ($qp_mode AND (strlen($word) > $length)) {
                    ...
                    if ('' !== $buf_o AND strlen($buf) > $length) {
...
//            $result .= static::$LE;
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] === $disposition_type) {
                ...
                if ((string) $cid !== '' AND $disposition === 'inline') {
    ...
    public function DKIM_QP($txt)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            $ord = ord($txt[$i]);
            if (((0x21 <= $ord) AND ($ord <= 0x3A)) || $ord === 0x3C || ((0x3E <= $ord) AND ($ord <= 0x7E))) {
                $line .= $txt[$i];
            } else {
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        $copiedHeaderFields = '';
        if ($this->DKIM_copyHeaderFields AND count($copiedHeaders) > 0) {
