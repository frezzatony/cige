...
class Clemdesign_Mask
{
  ...
  static function apply($inputValue, $maskExpression, $config = null)
  {
    ...
    if (Clemdesign_Mask::_startsWith($maskExpression, 'percent')) {
      $inputValue = Clemdesign_Mask::_checkInput($inputValue);
      $precision = Clemdesign_Mask::_getPrecision($maskExpression);
      $inputValue = Clemdesign_Mask::_checkInputPrecision($inputValue, $precision, '.');
      ...
      Clemdesign_Mask::_startsWith($maskExpression, 'separator') ||
      Clemdesign_Mask::_startsWith($maskExpression, 'dot_separator') ||
      Clemdesign_Mask::_startsWith($maskExpression, 'comma_separator')
    ) {
      // Clean input
      if(
      ...
      if (Clemdesign_Mask::_startsWith($maskExpression, 'separator')) {
        if (
        (strpos($inputValue,',') >= 0) &&
        Clemdesign_Mask::_endsWith($inputValue, ',') &&
      ...
      if (Clemdesign_Mask::_startsWith($maskExpression, 'dot_separator')) {
        if (
          (strpos($inputValue,',') > 0) &&
          strpos($inputValue,',') === strrpos($inputValue,',')
        ) {
      ...
      if (Clemdesign_Mask::_startsWith($maskExpression, 'comma_separator')) {
      ...
      } elseif (Clemdesign_Mask::_startsWith($maskExpression, 'dot_separator')) {
        if (preg_match("/[@#!$%^&*()_+|~=`{}\[\]:\s\";<>?\/]/", $inputValue)) {
          $inputValue = substr($inputValue, 0, strlen($inputValue) - 1);
        }
        $result = number_format($inputValue, $precision, ",", ".");
      } elseif (Clemdesign_Mask::_startsWith($maskExpression, 'comma_separator')) {
        $result = number_format($inputValue, $precision, ".", ",");
      }
...
//        var_dump("ME=".$maskCursor.", ME+1=".$maskCursorP1.", ME+2=".$maskCursorP2.", ME-1=".$maskCursorM1.", IS=".$inputSymbol.", Cur=".$cursor.", IV=".$inputValue);
        ...
        // 171
        if (Clemdesign_Mask::_checkSymbolMask($inputSymbol, $maskCursor) && ($maskCursorP1 === '?')) {
          $result .= $inputSymbol;
          $cursor += 2;
        } elseif ($maskCursorP1 === '*' && $multi && Clemdesign_Mask::_checkSymbolMask($inputSymbol, $maskCursor)) {
          $result .= $inputSymbol;
          $cursor += 3;
          $multi = false;
          //182
        } elseif (Clemdesign_Mask::_checkSymbolMask($inputSymbol, $maskCursor) && ($maskCursorP1 === '*')) {
          $result .= $inputSymbol;
          $multi = true;
        } elseif (($maskCursorP1 === '?') && Clemdesign_Mask::_checkSymbolMask($inputSymbol, $maskCursorP2)) {
          $result .= $inputSymbol;
          $cursor += 3;
          // 194
        } elseif (Clemdesign_Mask::_checkSymbolMask($inputSymbol, $maskCursor)) {
          if ($maskCursor === 'H') {
          ...
          $cursor++;
          // 272
        } elseif (Clemdesign_Mask::_findSpecialChar($maskCursor)) {
          $result .= $maskCursor;
          $cursor++;
          $i--;
        } elseif(
          Clemdesign_Mask::_findSpecialChar($inputSymbol) &&
          isset(Clemdesign_Mask::$maskAvailablePatterns[$maskCursor]) &&
          isset(Clemdesign_Mask::$maskAvailablePatterns[$maskCursor]["optional"]) &&
          Clemdesign_Mask::$maskAvailablePatterns[$maskCursor]["optional"] == true
        ) {
          $cursor++;
          $i--;
        } elseif ($maskCursorP1 === '*' &&
          Clemdesign_Mask::_findSpecialChar($maskCursorP2) &&
          Clemdesign_Mask::_findSpecialChar($inputSymbol) === $maskCursorP2 &&
          $multi
        ) {
          $cursor += 3;
          $result .= $inputSymbol;
    ...
    if (
      ...
      Clemdesign_Mask::_findSpecialChar(substr($maskExpression, -1))
    ) {
  ...
  {
    return (
      isset(Clemdesign_Mask::$maskAvailablePatterns[$maskSymbol]) &&
      Clemdesign_Mask::$maskAvailablePatterns[$maskSymbol]["pattern"] &&
      preg_match(Clemdesign_Mask::$maskAvailablePatterns[$maskSymbol]["pattern"], $inputSymbol)
    );
  }
  ...
  {
    $returnChar = false;
    foreach (Clemdesign_Mask::$maskSpecialCharacters as $char) {
      if ($inputSymbol === $char) {
        $returnChar = $char;
        break;
      }
