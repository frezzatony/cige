...
class Main extends CI_Controller{
    ...
    function __construct(){
        ...
        $this->profileUser = new Profiles_Users(
            array(
                'item'      =>  $idItem    
            )
        );
        ...
        if(is_array($arrToken)){
            ...
            $this->profileUser->validateToken($arrToken);
            
            $this->profileUser->set('token',$arrToken);
            if($arrToken['parent']){
                $arrToken['parent']['variable'] = $arrToken['variable'] ?? NULL;
                $this->profileUser->setParent($arrToken['parent']);
            }
        }
        
        //CLONE DE ITEM
        if($this->data->get('post.clone')){
            $this->profileUser->set('item',NULL);
        }
    ...
    public function delete(){
        ...
        $this->auth_model->login();
        if(!$this->profileUser->runActionUserPermissions(
            array(
        ...
        $arrResponseDelete = $this->profileUser->delete($arrPks);
        ...
        foreach($arrResponseDelete as $response){
            ...
            if(!$response['status']){
                ...
                $message = '[Cadastro]['.$this->profileUser->get('descricao_singular').'][Código:'.$response['pk'].']: Não pode ser excluído. ';
                ...
                $this->logger->setLog(
                    array(
                        'schema'        =>  $this->profileUser->get('data.schema').'_logs',
                        'table_log'     =>  $this->profileUser->get('data.schema').'_'.$this->profileUser->get('data.table'),
                        'item_id'       =>  $this->profileUser->get('item.value'),
                        'user_id'       =>  $this->data->get('user.id'),
                        'action'        =>  'DELETE',
                    )
                );
                ...
                $arrConsole[] = array(
                    'date_time' =>  date('d/m/Y|H:i:s'),
                    'message'   =>  '[Cadastro]['.$this->profileUser->get('descricao_singular').'][Código:'.$response['pk'].']: Foi excluído.'
                );   
            }
        }
    ...
    public function editItem($arrProp = array()){
        ...
        if(!$this->profileUser->runActionUserPermissions(
            array(
                'action'            =>  $this->profileUser->get('configs.actions.viewItems'),
                'user_id'           =>  $this->data->get('user.id'),
                'entity_id'         =>  $this->data->get('user.configs.entity'),
            )
        )){
        ...
        $htmlBody->add($this->profileUser->getHtmlDataDefaultBody());
        ...
        $htmlBody->add(
            array(
                'text'      =>  $this->profileUser->getActionMenuController(
                     array(
                        'pk_controller' =>  $this->profileUser->get('id'),
                        'user_id'       =>  $this->data->get('user.id'),
                        'entity_id'     =>  $this->data->get('user.configs.entity'),
                        'menu'          =>  include(dirname(__FILE__).'/../data/actionMenuControllerItem_perfis_usuarios.php'),
                    )
                ),
        ...
        include(dirname(__FILE__).'/../data/paneCadastroProfile.php');
        ...
        $buttonsHtmlData = $this->profileUser->getButtonsController(
            array(
                'pk_controller' =>  $this->profileUser->get('id'),
                'user_id'       =>  $this->data->get('user.id'),
                'entity_id'     =>  $this->data->get('user.configs.entity'),
                'menu'      =>  include(dirname(__FILE__).'/../data/itemFooterButtons_perfis_usuarios.php')
            )
        );
    ...
    public function getItemsGrid(){
        ...
        $arrListItemsKeys = array_column($this->profileUser->get('data.list_items.columns'),'id');
        $arrProp = $this->data->get('post');
        
        $arrProp['order'] =  $this->profileUser->getOrderBy();
        
        if($this->data->get('post.order.column')!==''){
            $arrProp['order'] =  $this->profileUser->getOrderBy(
                array(
                    array(
                        'id'        =>  $arrListItemsKeys[$this->data->get('post.order.column')] ?? $this->profileUser->get('data.list_items.columns')[0]['id'],
        ...
        $arrItems = $this->profileUser->getItems($arrProp);
        $arrData = array();
        
        foreach($arrItems['items'] as $item){
            
            $this->profileUser->setItem($item);
            ...
            foreach($arrListItemsKeys as $listKey){
                ...
                $columnValue = $this->profileUser->variables->get($listKey)->get('text');
                $columnValue = $columnValue ? $columnValue : $this->profileUser->variables->get($listKey)->get('value');
            ...
            $arrData[] = array(
                'data'      =>  $arrRow,
                'href'      =>  $this->parser->parse_string($this->profileUser->get('configs.url_view_element'), 
                    array(
                        'url_cadastro'  =>  $this->profileUser->get('url'),
    ...
    public function listItems($arrProp = array()){
        ...
        $this->template->set('actionMenu',$this->profileUser->getActionMenuController(
            array(
                'pk_controller' =>  $this->profileUser->get('id'),
                'user_id'       =>  $this->data->get('user.id'),
                'entity_id'     =>  $this->data->get('user.configs.entity'),
        ...
        $arrHtmlDataListItems = $this->profileUser->getHtmlDataListItems();    
    ...
    public function save($arrProp = array()){
        ...
        $arrDifferenceValues = $this->profileUser->getDifferenceValues(
            array(
                'method'    =>  'database'
            )
        );
        ...
        if(!$arrDifferenceValues AND $this->profileUser->get('item.value') != ''){
            Common::printJson(
                array(
                    'status'    =>  'info',
                    'id'        =>  $this->profileUser->get('item.value'),
                    'token'     =>  $this->profileUser->getToken(),
                    'console'   =>  array(
                        array(
                            'date_time' =>  date('d/m/Y|H:i:s'),
                            'message'   =>  '[Perfil de usuário]['.$this->profileUser->get('item.value').']:Não há alterações para registrar',
                            'type'      =>  'success',
                        )
                    ),
                    'messages'   =>  array(
        ...
        $validationErrors = $this->profileUser->variables->validate();
        ...
        $action = ($this->profileUser->get('item.value'))  ? 'UPDATE' : 'INSERT';
        
        $responseUpdate = $this->profileUser->update();
        ...
        $consoleResponse = '[Perfil de usuário][' .$responseUpdate.']: Foi ';
    ...
    private function setJsCadastros($arrProp = array()){
        ...
        $data = array(
            'item'  =>   $this->profileUser->get('item'),
            'url'   =>  current_url(),
        );
    ...
    private function setJsModal($arrProp = array()){
        ...
        $this->profileUser->append('javascript',$jsSetModal);
