...
class Main extends CI_Controller
{
    ...
    function change_user_entity(){
        ...
        $arrEntites = $this->entidades_model->getEntities(
            array(
                'build_tree'    =>  TRUE
            )
        );
        
        $arrUserEntities = $this->entidades_model->getUserEntities(
            array(
        ...
        Common::printJson(
            array(
                ...
                'body'      =>  $this->template->load($this->arrTemplate['template'],'entidades','TreeEntities_view',NULL,TRUE)
            )
        );
    ...
    function set_user_entity(){
        ...
        //altera
        $flag = $this->entidades_model->updateUserEntity(
            array(
                'user_id'   =>  $this->data->get('user.id'), 
                'entity_id' =>  $idEntity,
            ) 
    ...
    private function getNodesTree($arrProp = array()){
        ...
        foreach($arrProp['entities'] as $entity){
            ...
            if(($arrProp['user_entities']??NULL) AND array_search($entity['id'],array_column($arrProp['user_entities'],'id'))!==FALSE){
