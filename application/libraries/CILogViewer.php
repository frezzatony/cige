...
class CILogViewer {
    ...
    public function showLogs() {
        ...
        if(!is_null($dlFile) AND file_exists($this->logFolderPath . "/" . basename(base64_decode($dlFile))) ) {
        ...
        else if(is_null($fileName) AND !empty($files)) {
        ...
        if(!is_null($currentFile) AND file_exists($currentFile)) {

            $fileSize = filesize($currentFile);

            if(is_int($fileSize) AND $fileSize > self::MAX_LOG_SIZE) {
    ...
    private function processAPIRequests($command) {
        ...
        if($command === self::API_CMD_LIST) {
            ...
            if(is_null($file) || empty($file)) {
                ...
                $singleLine = !is_null($singleLine) AND ($singleLine === true || $singleLine === "true" || $singleLine === "1") ? true : false;
...
//               ]);
    ...
    private function prepareRawFileName($fileNameInBase64) {
        ...
        //let's determine what the current log file is
        if(!is_null($fileNameInBase64) AND !empty($fileNameInBase64)) {
