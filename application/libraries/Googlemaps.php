...
class Googlemaps {
	...
	function add_marker($params = array())
	{
		...
		if ($marker['zIndex']!="" AND is_numeric($marker['zIndex'])) {
			$marker_output .= ',
				zIndex: '.$marker['zIndex'];
		}
		if ($marker['animation']!="" AND (strtoupper($marker['animation'])=="DROP" || strtoupper($marker['animation']=="BOUNCE"))) {
			$marker_output .= ',
	...
	function add_polyline($params = array())
	{
		...
		if (count($polyline['points'])) {
			...
			if ($polyline['zIndex']!="" AND is_numeric($polyline['zIndex'])) {
				$polyline_output .= ',
	...
	function add_polygon($params = array())
	{
		...
		if ($polygon['zIndex']!="" AND is_numeric($polygon['zIndex'])) {
			$polygon_output .= ',
	...
	function add_circle($params = array())
	{
		...
		if ($circle['radius']>0 AND $circle['center']!="") {
			...
			if ($circle['zIndex']!="" AND is_numeric($circle['zIndex'])) {
				$circle_output .= ',
	...
	function add_rectangle($params = array())
	{
		...
		if ($rectangle['positionSW']!="" AND $rectangle['positionNE']!="") {
			...
			if ($rectangle['zIndex']!="" AND is_numeric($rectangle['zIndex'])) {
				$rectangle_output .= ',
	...
	function add_ground_overlay($params = array())
	{
		...
		if ($overlay['image']!="" AND $overlay['positionSW']!="" AND $overlay['positionNE']!="") {
	...
	function create_map()
	{
		...
		if ($this->maps_loaded == 0)
		{
			...
			if ($this->region!="" AND strlen($this->region)==2) { $apiLocation .= '&region='.strtoupper($this->region); }
		...
		if ($this->mapTypeControlStyle!="" AND (strtoupper($this->mapTypeControlStyle)=="DROPDOWN_MENU" || strtoupper($this->mapTypeControlStyle)=="HORIZONTAL_BAR")) {
		...
		if (count($this->styles) AND $this->stylesAsMapTypes) {
			$styles = 0;
			foreach ($this->styles as $style) {
		...
		if ($this->zoomControlStyle!="" AND (strtoupper($this->zoomControlStyle)=="SMALL" || strtoupper($this->zoomControlStyle)=="LARGE")) { array_push($zoomControlOptions, 'style: google.maps.ZoomControlStyle.'.strtoupper($this->zoomControlStyle)); }
		...
		if ((is_array($this->kmlLayerURL) AND count($this->kmlLayerURL)) || (!is_array($this->kmlLayerURL) AND $this->kmlLayerURL != ""))
		{
		...
		if ($this->directions) {
			...
			if ($this->directionsDraggable AND $this->directionsChanged!="") {
		...
		if ($this->places) {
			...
			if ($this->placesLocationSW!="" AND $this->placesLocationNE!="") { // if search based on bounds
			...
			if ($this->placesAutocompleteInputID != "")
			{
				$this->output_js_contents .= 'var autocompleteOptions = {
					';
				$autocompleteOptions = '';
				if ($this->placesAutocompleteBoundSW != "" AND $this->placesAutocompleteBoundNE != "")
				{
		...
		if ($this->directions AND $this->directionsStart!="" AND $this->directionsEnd!="") {
			if ($this->directionsStart=="auto" AND $this->directionsEnd=="auto") {
		...
		if ($this->directions) {
			...
			if ($this->region!="" AND strlen($this->region)==2) { 
				$this->output_js_contents .= ',region: '.strtoupper($this->region).'
					'; 
			}
			if (trim($this->directionsUnits)!="" AND (strtolower(trim($this->directionsUnits)) == "metric" || strtolower(trim($this->directionsUnits)) == "imperial")) { 
	...
	function is_lat_long($input)
	{
		...
		if (count($input)==2) {
			...
			if (is_numeric($input[0]) AND is_numeric($input[1])) { // is a lat long
				return true;
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		if ($this->region!="" AND strlen($this->region)==2) { $data_location .= "&region=".$this->region; }
		...
		if ($data->status=="OK") 
		{
			...
			if ($this->geocodeCaching) { // if we to need to cache this result
				if ($address != "" AND $lat != 0 AND $lng != 0)
				{
					$data = array(
