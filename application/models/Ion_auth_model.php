...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password($password, $identity = NULL)
	{
		...
		if ($algo !== FALSE AND $params !== FALSE)
		{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code === FALSE || ($user AND $user->id === $id))
		{
			$data = [
	...
	public function deactivate($id = NULL)
	{
		...
		else if ($this->ion_auth->logged_in() AND $this->user()->row()->id == $id)
		{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		else if (!$this->config->item('default_group', 'ion_auth') AND empty($groups))
		{
		...
		if (!isset($query->id) AND empty($groups))
		{
		...
		if (isset($default_group->id) AND empty($groups))
		{
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth') AND $this->config->item('track_login_ip_address', 'ion_auth'))
		{
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			$or_where_in = (!empty($group_ids) AND !empty($group_names)) ? 'or_where_in' : 'where_in';
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select(
            array(
                $this->tables['users_groups'].'.'.$this->join['groups'].' as id',
                $this->tables['groups'].'.nome',
                $this->tables['groups'].'.descricao',
                $this->tables['groups'].'.administrador',
                $this->tables['groups'].'.ativo',
            )
            )
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				if (isset($this->_cache_user_in_group[$user_id]) AND isset($this->_cache_user_in_group[$user_id][$group_id]))
				{
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) AND $this->identity_check($data[$this->identity_column]) AND $user->{$this->identity_column} !== $data[$this->identity_column])
		{
		...
		$this->db->set($this->tables['users'], $data, ['id' => $user->id]);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = [])
	{
		...
		if ($this->config->item('admin_group', 'ion_auth') === $group->name AND $group_name !== $group->name)
		{
	...
	protected function _get_hash_parameters($identity = NULL)
	{
		...
		if ($identity)
		{
			...
			if ($user_id AND $this->in_group($this->config->item('admin_group', 'ion_auth'), $user_id))
			{
				$is_admin = TRUE;
			}
		}
