...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        labelRemove: '<i class="fa fa-trash-o clickable"></i>',
        textConfirmDelete: 'This item will be deleted. Are you sure?',
        //iconPicker: { cols: 4, rows: 4, footer: false, iconset: "fontawesome4",arrowPrevIconClass: 'fa fa-arrow-left', arrowNextIconClass: 'fa fa-arrow-right', },
        listOptions: { 
    ...
    */
    ...
    function editItem($item) {
        ...
        iconPicker = $form.find('button.bsform-icon').eq(0)
        ...
        if (data.hasOwnProperty('icon')) {
            ...
            iconPicker.iconpicker('setIcon',data.icon.replace('fa ',''));
        } else{
    ...
    function TButtonGroup(cantEdit,cantDelete) {
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var isParent = ((typeof (v.children) !== "undefined") && ($.isArray(v.children) || typeof(v.children)=='object'));
            ...
            var $li = $('<li>').addClass('list-group-item pr-0 padding-4 padding-left-10 size-13');
            $li.data(itemObject);
            
            var $div = $('<div>').css('overflow', 'auto');
            var $i = $('<i>').addClass(v.icon);
            var $span = $('<span>').addClass('txt').append(v.text).css('margin-right', '5px');
            var $divbtn =  TButtonGroup(v.can_edit=='f',v.can_delete=='f');
    ...
    function TOpener(li){
        var opener = $('<span class="size-10">').addClass('sortableListsOpener ' + options.opener.openerClass).css(options.opener.openerCss)
                .on('mousedown touchstart', function (e){
                    var li = $(this).closest('li');
                    if (li.hasClass('sortableListsClosed')) {
                        li.iconOpen(options);
    ...
    this.update = function(){
        ...
        $.each($form.bsform({'method':'getSimpleValues'}),function(key, value){
            
            $cEl.data(key,value);
        })
        
        $cEl.children().children('i').removeClass(oldIcon).addClass('fa ' + $cEl.data('icon'));
    ...
    this.add = function(){
        ...
        var $li = $("<li>").data(data);
        $li.addClass('list-group-item pr-0 padding-4 padding-left-10').append(div);
        $main.append($li);
        MenuEditor.updateButtons($main);
        resetForm();
    };
    ...
    this.getString = function () {
        var obj = $.extend({},$main.sortableListsToJson());
        
        //obj = this.replaceSpecialChars(obj);
    ...
    this.replaceSpecialChars = function(obj){
        var plugin = this;
        
        $.each(obj,function(index,item){
            $.each(item,function(indexItem,valueItem){
                if(indexItem=='children'){
                   obj[index][indexItem] = plugin.replaceSpecialChars(obj[index][indexItem]);
                }
                else{
                    obj[index][indexItem] = valueItem.replace('{','\{').replace('{','\{')    
                }
            })
        });
